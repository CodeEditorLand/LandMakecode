...
export class Simulator {
	...
	public static createOrShow(extCtx: vscode.ExtensionContext) {
		...
		if (Simulator.simconsole) {
			...
			Simulator.simconsole =
				vscode.window.createOutputChannel("MakeCode");
		}
		...
		const panel = vscode.window.createWebviewPanel(
			Simulator.viewType,
			vscode.l10n.t("Microsoft MakeCode Simulator"),
			{
...
async function getSimLoaderHtmlAsync() {
	...
	return index
		.replace(
			/<\s*script\s+type="text\/javascript"\s+src="([^"]+)"\s*>\s*<\/\s*script\s*>/g,
			(substring, match) => {
				if (match === "loader.js") {
					return `
