...
export function createVsCodeHost(): Host {
	return {
		readFileAsync: async (p, e) =>
			readFileAsync(rmFolderPrefix(p), e as any),
		writeFileAsync: async (p, c, e) =>
			writeFileAsync(rmFolderPrefix(p), c, e),
		...
		createLanguageServiceAsync: async (editor) =>
			new BrowserLanguageService(editor),
...
export async function writeFileAsync(
	...
	await vscode.workspace.fs.writeFile(resolvePath(path), content);
}
...
export async function httpRequestCoreAsync(options: HttpRequestOptions) {
	...
	resp.headers.forEach(
		(value, key) => (res.headers[key.toLowerCase()] = value),
	);
...
function base64EncodeBufferAsync(
